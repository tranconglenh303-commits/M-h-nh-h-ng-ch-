...
import { ShoppingCart, PhoneCall, HeartPulse, Server, Plane, Factory, ArrowRight, CheckCircle2, Zap } from 'lucide-react';
...
export const RealWorldApplications: React.FC = () => {
  ...
  const applications: ApplicationItem[] = [
    {
      ...
      problem: 'Ở mô hình truyền thống (mỗi quầy 1 hàng), khách hay gặp cảnh hàng bên cạnh đi nhanh hơn hoặc không may đứng sau một khách gặp sự cố thẻ/đổi trả hàng khiến cả hàng bị tắc (hiện tượng Jockeying và bất công tâm lý).',
      solution: 'Chuyển sang thiết kế 1 hàng rào ziczac chung (Snake Line). Khách đầu hàng sẽ tiến vào bất kỳ quầy nào vừa trống.',
      mathInsight: 'Về mặt toán học, 1 hàng chung M/M/c loại bỏ hoàn toàn thời gian quầy bị bỏ trống lãng phí (Server Idle Time) khi vẫn còn người đang chờ ở hàng khác. Độ lệch chuẩn thời gian chờ giảm tới 65%, tâm lý khách hàng hài lòng hơn vượt bậc.',
      keyTakeaways: [
        ...
        'Được áp dụng thành công tại các chuỗi bán lẻ toàn cầu như Zara, Uniqlo, Best Buy, Sephora.',
      ],
    },
    {
      id: 'callcenter',
      ...
      problem: 'Doanh nghiệp cần xác định số lượng điện thoại viên ca trực sao cho không để khách chờ quá lâu dẫn đến ngắt máy (Reneging), đồng thời không lãng phí chi phí nhân sự rảnh rỗi.',
      solution: 'Sử dụng công thức Erlang C để tính xác suất một cuộc gọi phải chờ quá t giây: P(Wait > t) = C(c, r) · exp(-(c·μ - λ)·t).',
      mathInsight: 'Quy chuẩn SLA phổ biến ngành viễn thông: 80% cuộc gọi được bắt máy trong vòng 20 giây (SLA 80/20). Nếu λ = 120 cuộc/giờ, μ = 15 cuộc/giờ (4 phút/cuộc), cần tối thiểu 11 agent để đạt SLA 86%.',
      keyTakeaways: [
        'Dự báo chính xác số điện thoại viên cần phân ca theo từng khung giờ trong ngày.',
        'Giảm tỷ lệ khách hàng dập máy vì bực bội (Abandonment Rate < 3%).',
    ...
    },
    {
      ...
      problem: 'Khoa cấp cứu không thể áp dụng FIFO vì bệnh nhân đau tim, tai nạn giao thông nặng cần cấp cứu ngay, không thể đợi theo thứ tự đến sau người bị bong gân.',
      solution: 'Quy trình Triage 5 mức độ (ESI - Emergency Severity Index). Bệnh nhân nhóm 1 (Nguy kịch) được bác sĩ tiếp nhận ngay lập tức, ngắt quãng mọi ca phục vụ không khẩn cấp.',
      mathInsight: 'Mô hình hàng chờ ưu tiên ngắt quãng (Preemptive Priority) chứng minh thời gian chờ của nhóm ưu tiên cao nhất W_q1 = λ_1 / [μ (μ - λ_1)] hoàn toàn không phụ thuộc vào số lượng bệnh nhân nhóm nhẹ đang xếp hàng!',
      keyTakeaways: [
        'Cứu sống tính mạng bệnh nhân nặng trong "Khung giờ vàng" (Golden Hour).',
        'Phân luồng phòng khám nhanh (Fast-track) cho các ca bệnh nhẹ để giải phóng mặt bằng phòng chờ.',
    ...
    },
    {
      ...
      problem: 'Khi lượng truy cập tăng đột biến (Flash Sale, Black Friday), máy chủ web nhận hàng chục ngàn HTTP request mỗi giây. Nếu không có bộ đệm giới hạn K, RAM và CPU sẽ cạn kiệt, dẫn đến sập toàn bộ cụm máy chủ (Crash).',
      solution: 'Thiết lập hàng đợi có kích thước giới hạn (Bounded Queue M/M/1/K). Các request vượt ngưỡng dung lượng K sẽ nhận ngay phản hồi HTTP 429 Too Many Requests hoặc chuyển vào Kafka Broker để xử lý bất đồng bộ (Backpressure).',
      mathInsight: 'Tốc độ thực tế λ_eff = λ · (1 - P_K) giữ cho hệ thống luôn chạy ở công suất bão hòa tối đa mà không bị tràn bộ nhớ (Out-Of-Memory).',
      keyTakeaways: [
    ...
    },
    {
      ...
      problem: 'Trạm thu phí thủ công dùng tiền mặt khiến xe dừng chờ lâu, thời gian thanh toán dao động lớn từ 10s đến 60s gây ùn tắc hàng cây số vào giờ cao điểm.',
      solution: 'Chuyển sang làn thu phí không dừng ETC (Electronic Toll Collection) và cổng soát vé tự động Auto-gate tại sân bay.',
      mathInsight: 'Theo công thức Pollaczek-Khinchine, khi phương sai thời gian phục vụ triệt tiêu về 0 (σ² = 0, mô hình M/D/c), độ dài hàng chờ và thời gian chờ giảm chính xác 50% so với mô hình ngẫu nhiên M/M/c dù tốc độ trung bình không đổi!',
      keyTakeaways: [
        ...
        'Giảm tiêu hao nhiên liệu và khí thải ô nhiễm môi trường do xe chạy rà rà.',
      ],
    },
    {
      id: 'factory',
      ...
      problem: 'Sản phẩm dở dang (Work In Progress - WIP) ứ đọng khổng lồ giữa các công đoạn sản xuất, làm chôn vốn lưu động và kéo dài thời gian hoàn thành đơn hàng (Lead Time).',
      solution: 'Áp dụng Lý thuyết Điểm nghẽn (Theory of Constraints) và hệ thống thẻ Kanban kéo (Pull System) để giới hạn WIP tại từng trạm.',
      ...
      keyTakeaways: [
        'Xác định trạm có hệ số sử dụng cao nhất ρ_max để ưu tiên đầu tư cải tiến.',
  ...
  return (
    <section id="applications" className="py-8">
      ...
      {/* Selected Application Deep-Dive Card */}
