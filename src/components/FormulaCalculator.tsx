...
import { Calculator, ArrowRight, CheckCircle, Info, BookOpen, Layers } from 'lucide-react';

export const FormulaCalculator: React.FC = () => {
  const [selectedModel, setSelectedModel] = useState<KendallModelType>('M/M/1');

  // Input states
  const [lambda, setLambda] = useState<number>(8); // arrivals/hour or /min
  const [mu, setMu] = useState<number>(10); // service rate
  const [c, setC] = useState<number>(2); // servers for M/M/c
  const [K, setK] = useState<number>(5); // capacity for M/M/1/K
  const [sigma, setSigma] = useState<number>(0.05); // standard deviation for M/G/1

  // Calculate results based on model
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 backdrop-blur">
            ...
            <div className="mb-4">
              <label className="text-xs text-slate-300 font-medium block mb-1.5">
                Tốc độ phục vụ mỗi quầy (<MathView formula="\mu" />): khách/giờ (hoặc khách/phút)
              </label>
              <div className="flex items-center gap-3">
                <input
                  type="number"
            ...
            {/* Model specific: sigma for M/G/1 */}
            {selectedModel === 'M/G/1' && (
              <div className="mb-4">
                ...
                <span className="text-[10px] text-slate-500 block mt-1">
                  Nếu σ = 0: Trở thành mô hình M/D/1 (thời gian phục vụ cố định).
                </span>
              </div>
            )}
        ...
        {/* Right Column: Step-by-Step Breakdown (8 cols) */}
          ...
          <div className="space-y-4">
            {steps.map((st) => (
              ...
              >
                ...
                {/* Numerical substitution & explanation */}
