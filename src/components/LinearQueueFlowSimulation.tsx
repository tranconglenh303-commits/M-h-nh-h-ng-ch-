...
import { Play, Pause, RotateCcw, UserPlus, Sliders, Zap, HelpCircle } from 'lucide-react';
...
export const LinearQueueFlowSimulation: React.FC = () => {
    ...
    const render = (timestamp: number) => {
      ...
      particles.forEach((p) => {
        if (p.state === 'in_service') {
          ...
          if (p.serviceRemainingSec <= 0) {
            // Service completed! Move to departing state
            p.state = 'departing';
      ...
      particles.forEach((p) => {
        if (p.state === 'departing') {
          // Move straight to the right off screen
          p.x += 160 * dt;
        } else {
      ...
      ctx.fillStyle = '#0b1120'; // deep slate
      ctx.fillRect(0, 0, width, height);

      // Draw Entrance / Flow Guideline on left
      ...
      // Label: "Hàng chờ" exactly like in the user's reference image
      ctx.fillStyle = '#94a3b8';
      ctx.font = '500 13px "Plus Jakarta Sans", sans-serif';
      ctx.textAlign = 'left';
      ctx.fillText('Hàng chờ', queueLineStartX + 10, centerY + 34);

      // Draw Server Boxes ("Server")
      serverPositions.forEach((sPos, idx) => {
        ...
        // Progress arc around busy server
      ...
      // Draw Departure Line to the right
      ctx.strokeStyle = '#1e293b';
      ctx.lineWidth = 2;
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
      ...
      particlesRef.current.forEach((p) => {
        ...
        // ID number or state marker inside dot
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/90 shadow-2xl overflow-hidden backdrop-blur">
      {/* Top Banner with Stats exact reference layout */}
      <div className="p-5 md:p-6 border-b border-slate-800 bg-slate-950/70">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Real-time stats counters matching image reference */}
      ...
      <div className="relative w-full h-[260px] md:h-[300px] bg-slate-950">
        <canvas
          ...
          title="Bấm để xem các hạt khách hàng di chuyển từ trái sang phải"
        />
      ...
      {/* Legend & Controls Toolbar (matching image reference legend) */}
      <div className="p-4 md:p-5 bg-slate-950/90 border-t border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        {/* Legend from user image: Khách đến, Đang chờ, Đang phục vụ */}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Thêm 1 khách hàng mới vào dòng chảy ngay lập tức"
          >
      ...
      {/* Quick Parameter Sliders below simulation */}
