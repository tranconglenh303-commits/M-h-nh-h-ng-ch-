...
import { TrendingDown, DollarSign, Award, ArrowUpRight, BarChart2 } from 'lucide-react';

export const EconomicOptimization: React.FC = () => {
  const [lambda, setLambda] = useState<number>(30); // 30 customers/hour
  const [mu, setMu] = useState<number>(12); // 12 customers/hour per server
  const [costService, setCostService] = useState<number>(100); // 100k VND or $100/server/hour
  const [costWait, setCostWait] = useState<number>(150); // 150k VND or $150/customer-wait/hour
  ...
  return (
    <section id="economics" className="py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div>
            ...
            <div className="mb-4">
              ...
              <span className="text-[10px] text-slate-500 block mt-1">
                Bao gồm lương nhân viên, điện, khấu hao thiết bị quầy.
              </span>
            </div>
            ...
            <div className="mb-4">
              ...
              />
              <span className="text-[10px] text-slate-500 block mt-1">
                Bao gồm sự khó chịu, rủi ro khách bỏ hàng, sụt giảm uy tín thương hiệu.
              </span>
            </div>
          </div>
        ...
        {/* Visual Chart & Table (8 cols) */}
        <div className="lg:col-span-8 flex flex-col gap-4">
          {/* Visual Cost Curves Graph */}
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 backdrop-blur">
            ...
            <div className="space-y-3 pt-2">
              {results.map((res) => {
                const totalPercent = Math.min(100, (res.totalCost / maxTotalCost) * 100);
                const servicePercent = (res.costService / res.totalCost) * 100;
                const waitPercent = (res.costWaiting / res.totalCost) * 100;
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950/80 text-slate-400 font-mono border-b border-slate-800">
                  <tr>
                    <th className="py-2.5 px-3">Số Quầy (c)</th>
                    <th className="py-2.5 px-3">Hiệu Suất (ρ)</th>
                    <th className="py-2.5 px-3">Khách Trong Hệ Thống (L)</th>
                    <th className="py-2.5 px-3">Chi Phí Quầy (Cs·c)</th>
                    <th className="py-2.5 px-3">Tổn Thất Chờ (Cw·L)</th>
                    <th className="py-2.5 px-3 text-right">Tổng Chi Phí (TC)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60 font-mono">
                  {results.map((res) => (
                    <tr
                      key={res.servers}
                      className={res.isOptimal ? 'bg-emerald-950/20 font-bold text-white' : 'text-slate-300 hover:bg-slate-800/40'}
                    >
                      <td className="py-2 px-3 flex items-center gap-1.5">
                        <span>{res.servers} quầy</span>
                        {res.isOptimal && <span className="text-emerald-400">✓</span>}
                      </td>
                      <td className="py-2 px-3">{(res.utilization * 100).toFixed(1)}%</td>
                      <td className="py-2 px-3">{res.L.toFixed(2)}</td>
                      <td className="py-2 px-3 text-purple-300">{res.costService}.000đ</td>
                      <td className="py-2 px-3 text-amber-300">{res.costWaiting.toFixed(0)}.000đ</td>
                      <td className={`py-2 px-3 text-right ${res.isOptimal ? 'text-emerald-400 text-sm' : 'text-slate-200'}`}>
                        {res.totalCost.toFixed(0)}.000đ
                      </td>
                    </tr>
                  ))}
                </tbody>
